...
CBaseSocket::~CBaseSocket()
{
	log_info("CBaseSocket::~CBaseSocket, socket=%d", m_socket);
}
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	{
		log_error("socket failed, err_code=%d\n", _GetErrorCode());
		return NETLIB_ERROR;
	}
	...
	if (ret == SOCKET_ERROR)
	{
		log_error("bind failed, err_code=%d", _GetErrorCode());
	...
	if (ret == SOCKET_ERROR)
	{
		log_error("listen failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_ERROR;
	}
	...
	log_info("CBaseSocket::Listen on %s:%d", server_ip, port);
...
{
	log_info("CBaseSocket::Connect, server_ip=%s, port=%d", server_ip, port);
	...
	if (m_socket == INVALID_SOCKET)
	{
		log_error("socket failed, err_code=%d", _GetErrorCode());
	...
	{	
		log_error("connect failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_INVALID_HANDLE;
	}
...
int CBaseSocket::Send(void* buf, int len)
{
	...
	if (ret == SOCKET_ERROR)
	{
		...
		{
			log_error("!!!send failed, error code: %d", err_code);
		}
	}
...
void CBaseSocket::SetSendBufSize(uint32_t send_size)
{
	...
	log_info("socket=%d send_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size)
{
	...
	log_info("socket=%d recv_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		log_error("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		log_error("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
		log_error("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr)
{
	...
	if (pAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		if (host == NULL)
		{
			log_error("gethostbyname failed, ip=%d", ip);
			return;
		}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		log_info("AcceptNewSocket, socket=%d from %s:%d\n", fd, ip_str, port);
